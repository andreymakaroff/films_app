  ...
  render() {
    ...
    return (
      <View style={Style.container}>
        ...
        {/*<Text style={Style.text}>The weather temperature is: {JSON.stringify(temperature)}</Text>*/}
        <Text style={Style.text}>
          The weather temperature is: {JSON.stringify(this.props.navigate)}
        </Text>
        <Text style={Style.text}>{this.props.isHot ? "It's pretty hot!" : ''}</Text>
        <Text style={Style.text}>{this.props.temperatureErrorMessage}</Text>
        <Button onPress={() => this.props.navigate('MainScreen2')} title="Refresh" />
      </View>
    )
  }
}
